...
# --- Zellij workbench resources ------------------------------------------------
step "Zellij workbench resources"
info "The zellij container is the dev workbench: coding agents (Claude Code,"
info "Codex, opencode, …) compile and run tests inside it. Give it most of"
info "the VPS resources — the rest of the stack idles at well under 1 GB."
TOTAL_MEM_MB=$(free -m 2>/dev/null | awk '/^Mem:/{print $2}' || true)
CPU_COUNT=$(nproc 2>/dev/null || getconf _NPROCESSORS_ONLN 2>/dev/null || true)
SUGGESTED_MEM="2G"
if [[ -n "${TOTAL_MEM_MB:-}" ]]; then
    # Leave ~3 GB headroom for the OS + the other containers; floor at 1 GB.
    mem_mb=$(( TOTAL_MEM_MB - 3072 ))
    (( mem_mb < 1024 )) && mem_mb=1024
    SUGGESTED_MEM="${mem_mb}M"
...
if ask_yes_no "Set up Telegram notifications for Watchtower?" "y"; then
    ...
    # Empty value disables Watchtower notifications (see docker-compose.yml).
...
# Helper: escape single quotes for safe .env sourcing ('val' → 'val'\''s')
_qv() { printf "'%s'" "${1//\'/\'\\\'\'}"; }

# Keep a backup of the previous .env so a re-run can never destroy config.
if [[ -f "$ENV_FILE" ]]; then
    cp -p "$ENV_FILE" "${ENV_FILE}.bak"
    info "Previous .env backed up to .env.bak"
fi

cat > "$ENV_FILE" <<EOF
# Generated by scripts/install.sh on $(date -u +%FT%TZ)
DOMAIN=$(_qv "$DOMAIN")
BRAND_NAME=$(_qv "$BRAND_NAME")
TZ=$(_qv "$TZ")
ACME_EMAIL=$(_qv "$ACME_EMAIL")
TELEGRAM_BOT_TOKEN=$(_qv "$TELEGRAM_BOT_TOKEN")
TELEGRAM_CHAT_ID=$(_qv "$TELEGRAM_CHAT_ID")
WATCHTOWER_NOTIFICATIONS=$(_qv "$WATCHTOWER_NOTIFICATIONS")
SMTP_HOST=$(_qv "${SMTP_HOST:-}")
SMTP_PORT=$(_qv "${SMTP_PORT:-}")
SMTP_USERNAME=$(_qv "${SMTP_USERNAME:-}")
SMTP_PASSWORD=$(_qv "${SMTP_PASSWORD:-}")
SMTP_SENDER=$(_qv "${SMTP_SENDER:-}")
ZELLIJ_MEM_LIMIT=$(_qv "${ZELLIJ_MEM_LIMIT:-2G}")
ZELLIJ_CPU_LIMIT=$(_qv "${ZELLIJ_CPU_LIMIT:-2.0}")
EOF
# Preserve optional overrides carried over from a previous .env (sourced above)
# that the Q&A doesn't ask about — rewriting the file must not drop them.
for extra in ZELLIJ_VERSION LAB_STATE_DIR SMTP_SCHEME \
             ALERT_DISK_PCT ALERT_MEM_PCT ALERT_LOAD HEARTBEAT_HOURS \
             ALERT_SSH ALERT_AUTHELIA ALERT_COOLDOWN; do
    if [[ -n "${!extra:-}" ]]; then
        echo "${extra}=$(_qv "${!extra}")" >> "$ENV_FILE"
    fi
done
chmod 600 "$ENV_FILE"
ok ".env written"
